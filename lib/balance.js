...
const fail = (reason, extra = {}) => ({ success: false, reason, balance: null, before: null, ...extra });

/**
 * @returns {{success:boolean, reason:string, balance:number|null, before:number|null, message?:string}}
 */
export async function changeBalance(uid, delta, { allowNegative = false, database } = {}) {
  ...
  try {
    ...
    if (!res.committed) return fail(DATABASE_ERROR, { before, message: 'Transaksi saldo tidak ter-commit' });
    if (decision === MALFORMED_BALANCE) return fail(MALFORMED_BALANCE, { message: 'Nilai saldo di database bukan angka' });
    if (decision === INSUFFICIENT_BALANCE) return fail(INSUFFICIENT_BALANCE, { balance: toAmount(res.snapshot.val()), before });
    return { success: true, reason: BALANCE_OK, balance: toAmount(res.snapshot.val()), before };
  } catch (e) {
    return fail(DATABASE_ERROR, { before, message: String(e && e.message).slice(0, 200) });
  }
}
